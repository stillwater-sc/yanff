...
func GetL2ACLFromJSON(filename string) (*L2Rules, error) {
	...
	if err != nil {
		msg := common.LogError(common.Debug, "JSON error during rules parsing: ", err)
		return nil, common.NFError{Code: common.CodeParseRuleJSONErr, Message: msg}
	}
	// Parse Rules
	err = rawL2Parse(&rawRules, &rules)
	return &rules, err
}
...
func GetL2ACLFromORIG(filename string) (*L2Rules, error) {
	...
	if err != nil {
		msg := common.LogError(common.Debug, "File error during rules parsing: ", err)
	...
	for scanner.Scan() {
		...
		if len(lines) == 3 {
			...
			msg := common.LogError(common.Debug, "Incomplete 3-tuple for rule parsing")
...
func GetL3ACLFromORIG(filename string) (*L3Rules, error) {
	...
	for scanner.Scan() {
		...
		if len(lines) == 5 {
			...
			msg := common.LogError(common.Debug, "Incomplete 5-tuple for rule parsing")
			return nil, common.NFError{Code: common.CodeParseRuleError, Message: msg}
		}
...
func rawL2Parse(rules *rawL2Rules, jp *L2Rules) error {
	...
	for i := 0; i < len(jup); i++ {
		var e error
		jp.eth[i].OutputNumber, e = parseRuleResult(jup[i].Rule)
		if e != nil {
			return e
		}
		...
		if jup[i].Source != "ANY" {
			...
			if err != nil {
				msg := common.LogError(common.Debug, "Incorrect source MAC: ", jup[i].Source)
		...
		if jup[i].Destination != "ANY" {
			...
			if err != nil {
				msg := common.LogError(common.Debug, "Incorrect destination MAC: ", jup[i].Destination)
				return common.NFError{Code: common.CodeIncorrectArgInRules, Message: msg}
			}
		...
		case "arp", "Arp", "ARP", "0x0806":
			...
			jp.eth[i].IDMask = 0xffff
		default:
			msg := common.LogError(common.Debug, "Incorrect L3 protocol ID: ", jup[i].ID)
			return common.NFError{Code: common.CodeIncorrectArgInRules, Message: msg}
		}
	}
	return nil
	...
	for i := 0; i < len(jup); i++ {
		...
		case "icmp", "ICMP", "Icmp", "0x01", "1":
			...
			if jup[i].SrcPort != "ANY" || jup[i].DstPort != "ANY" {
				msg := common.LogError(common.Debug, "Incorrect request: for ICMP rule Source port and Destination port should be ANY")
				return common.NFError{Code: common.CodeIncorrectArgInRules, Message: msg}
			}
		default:
			msg := common.LogError(common.Debug, "Incorrect L4 protocol ID: ", jup[i].ID)
			return common.NFError{Code: common.CodeIncorrectArgInRules, Message: msg}
		}
		var err error
		// Parse L4 ports
		...
		if srcLen == 0 {
			...
			if dstLen == 0 {
				...
				msg := common.LogError(common.Debug, "Incorrect request: IPv4 + IPv6 in one rule")
				return common.NFError{Code: common.CodeIncorrectArgInRules, Message: msg}
			}
			temp4.OutputNumber, err = parseRuleResult(jup[i].OutputNumber)
			if err != nil {
...
func parseL4Port(port string) (uint16, uint16, bool, error) {
	...
	if len(s) != 2 {
		msg := common.LogError(common.Debug, "Incorrect port: ", port)
	...
	if errMin != nil || errMax != nil {
		msg := common.LogError(common.Debug, "Incorrect request: cannot parse Min and Max port values in ", port)
		return 0, 0, false, common.NFError{Code: common.CodeIncorrectArgInRules, Message: msg}
	}
	if tMin > tMax {
		msg := common.LogError(common.Debug, "Incorrect request: minPort > maxPort", port)
		return 0, 0, false, common.NFError{Code: common.CodeIncorrectArgInRules, Message: msg}
	}
	return uint16(tMin), uint16(tMax), valid, nil
}
...
func parseRuleResult(rule string) (uint, error) {
	...
	default:
		...
		if err != nil {
			msg := common.LogError(common.Debug, "Incorrect rule: ", rule)
